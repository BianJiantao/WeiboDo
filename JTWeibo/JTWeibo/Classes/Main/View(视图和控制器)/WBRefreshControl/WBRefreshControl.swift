...
/// 刷新状态切换的临界点
fileprivate let WBRefreshOffset: CGFloat = 122
...
class WBRefreshControl: UIControl {
    ...
    fileprivate lazy var refrershView: WBRefreshView = WBRefreshView.refreshView()
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if sv.isDragging {
            if height > WBRefreshOffset && refrershView.refreshStatus == .Normal{
                print("放手刷新")
                refrershView.refreshStatus = .Pulling
            }else if height <= WBRefreshOffset && refrershView.refreshStatus == .Pulling{
                print("继续拽")
    ...
    func beginRefreshing(){
       print("开始刷新")
        ...
        inset1.top += WBRefreshOffset
        sv.contentInset = inset1
        
        refrershView.parentViewHeight = WBRefreshOffset
    }
...
extension WBRefreshControl {
    ...
    fileprivate func steupUI(){
       backgroundColor = superview?.backgroundColor
        ...
        addConstraint(NSLayoutConstraint(item: refrershView,
                                         attribute: .centerX,
                                         relatedBy: .equal,
                                         toItem: self,
                                         attribute: .centerX,
        ...
        addConstraint(NSLayoutConstraint(item: refrershView,
                                         ...
                                         multiplier: 1.0,
                                         constant: refrershView.bounds.width))
        addConstraint(NSLayoutConstraint(item: refrershView,
                                         attribute: .height,
                                         relatedBy: .equal,
                                         toItem: nil,
                                         attribute: .notAnAttribute,
                                         multiplier: 1.0,
                                         constant: refrershView.bounds.height))
