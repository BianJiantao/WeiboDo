...
class WBEmoticonManager {
    ...
    /// 表情包懒加载数组
    lazy var packageArr = [WBEmoticonPackage]()
    
    /// 表情素材的 bundle
    lazy var bundle: Bundle = {
        let path = Bundle.main.path(forResource: "WBEmoticon.bundle", ofType: nil)
...
fileprivate extension WBEmoticonManager {
    ...
    func loadPackages() {
        ...
        guard let path = Bundle.main.path(forResource: "WBEmoticon", ofType: "bundle"),
            let bundle = Bundle(path: path),
            let plistPath = bundle.path(forResource: "emoticons", ofType: "plist"),
            let array = NSArray(contentsOfFile: plistPath) as? [[String:String]],
            let modelArr = NSArray.yy_modelArray(with: WBEmoticonPackage.self, json: array) as? [WBEmoticonPackage] else {
                return
        }
