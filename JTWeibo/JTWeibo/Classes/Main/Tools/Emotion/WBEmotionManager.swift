...
class WBEmotionManager {
    static let shared = WBEmotionManager()
    
    /// 表情包懒加载数组
    lazy var packageArr = [WBEmotionPackage]()
    
    /// 表情素材的 bundle
    lazy var bundle: Bundle = {
        let path = Bundle.main.path(forResource: "WBEmotion.bundle", ofType: nil)
...
extension WBEmotionManager {
    ...
    func emotionString(string: String,font: UIFont) -> NSAttributedString {
        ...
        for m in matches.reversed() {
            ...
            //使用subStr 查找对应的表情符号
            if let em = WBEmotionManager.shared.findEmotion(string: subStr) {
    ...
    /// - 如果找到返回表情模型
    /// - 否则 返回nil
    func findEmotion(string:String?) -> WBEmotion? {
        //遍历表情包 OC中过滤数组使用[谓词] swift  更简单
        for p in packageArr {
            //方法1.在表情数组中过滤 string
//            let result = p.emotionArr.filter({ (em) -> Bool in
...
// MARK: - 表情包数据处理
fileprivate extension WBEmotionManager {
    
    func loadPackages() {
        //读取 emotions.plist
        //只要按照 Bundle 默认的结构目录设定，就可以直接读取 Resources 目录的文件
        guard let path = Bundle.main.path(forResource: "WBEmotion", ofType: "bundle"),
            let bundle = Bundle(path: path),
            let plistPath = bundle.path(forResource: "emotions", ofType: "plist"),
            let array = NSArray(contentsOfFile: plistPath) as? [[String:String]],
            let modelArr = NSArray.yy_modelArray(with: WBEmotionPackage.self, json: array) as? [WBEmotionPackage] else {
                return
        }
